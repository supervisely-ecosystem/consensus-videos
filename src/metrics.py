...
from supervisely import ProjectMeta, VideoAnnotation, VideoFigure, TagCollection
...
class ComputeMetricsReq:
    def __init__(
        self,
        gt_video_info: VideoInfo,
        pred_video_info: VideoInfo,
        ...
        self.gt_video_ann = gt_video_ann
        self.pred_video_ann = pred_video_ann
...
def compute_metrics(
    ...
    gt_video_info = request.gt_video_info
    pred_video_info = request.pred_video_info
    gt_video_ann = request.gt_video_ann
    ...
    try:
        ...
        if frame_count != pred_video_info.frames_count:
            raise MetricsException(
                message="Ground truth video and Prediction video infos have frames count."
            )
        ...
        for frame_n in range(1, gt_video_info.frames_count + 1):
            ...
            for idx in nonseg_gt_figures_idxs:
                ...
                nonseg_gt_class_to_indices.setdefault(fig.video_object.obj_class.name, []).append(
                    idx
                )
            nonseg_pred_class_to_indices = {}
            for idx in nonseg_pred_figures_idxs:
                fig = pred_frame_figures[idx]
                nonseg_pred_class_to_indices.setdefault(fig.video_object.obj_class.name, []).append(
                    idx
                )
            ...
            for idx in seg_gt_figures_idxs:
                fig = gt_frame_figures[idx]
                seg_gt_class_to_indices.setdefault(fig.video_object.obj_class.name, []).append(idx)
            seg_pred_class_to_indices = {}
            for idx in seg_pred_figure_idxs:
                fig = pred_frame_figures[idx]
                seg_pred_class_to_indices.setdefault(fig.video_object.obj_class.name, []).append(
                    idx
                )
            ...
            if np.any(frame_errors_canvas):
                difference_geometries.append(Bitmap(frame_errors_canvas))
            else:
...
def calculate_exam_report(
    ...
    request = ComputeMetricsReq(
        ...
        pred_video_info=pred_video_info,
        gt_video_ann=gt_video_ann,
        pred_video_ann=pred_video_ann,
